...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  ...
  try {
    ...
    const { customerId, vehicleId, notes, items } = body
    ...
    const [result] = await db.query<any>(
      `INSERT INTO quotes
         (store_id, prepared_by_staff_id, customer_id, vehicle_id, quote_number,
      ...
      [storeId, techId, customerId, vehicleId, quoteNumber, notes ?? null],
    )
