...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  ...
  try {
    ...
    const allowed = ['name', 'sku', 'category', 'unit_price', 'description', 'active']
    ...
    const [result] = await db.query<any>(`UPDATE catalog SET ${set} WHERE id = ?`, values)
    if (result.affectedRows === 0) return notFound('Catalog item')

    const [rows] = await db.query<any[]>('SELECT * FROM catalog WHERE id = ? LIMIT 1', [id])
    return ok(rows[0])
  } catch (err) {
