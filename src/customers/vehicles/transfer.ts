...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  ...
  const customerId = Number(event.pathParameters?.customerId)
  ...
  try {
    ...
    if (!ownership) return notFound('Vehicle')
    ...
    if (buyer.id === customerId) return validationError('The buyer is already the current owner.')
    ...
    try {
      ...
      await conn.query(
        ...
        [odometerAtRelease ?? null, vehicleId, customerId],
      )
    ...
    return ok({ transferred: true, vehicleId, previousOwnerId: customerId, newOwnerId: buyer.id })
  } catch (err) {
