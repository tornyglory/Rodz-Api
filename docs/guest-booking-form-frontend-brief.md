...
| 4 | Pick model | `GET {CATALOG_BASE}/public/vehicle-catalog/models?year=&make=` |
| 5 | Pick series (skip if empty) | `GET {CATALOG_BASE}/public/vehicle-catalog/series?year=&make=&model=` |
...
- `rego` (string, uppercased)
- `regoState` — one of: `VIC`, `NSW`, `QLD`, `SA`, `WA`, `TAS`, `NT`, `ACT`
- `km` (integer) — sent to step 6b's overview call for the personalised intro
- Optional: `fuelType`, `transmission`, `series` — for the vehicle upsert on submit
...
- `vehicle.year` (1900–2100), `make`, `model`, `rego`, `regoState`
